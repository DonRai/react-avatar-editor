...
class Editor extends React.Component {
    ...
    paintImage(context, image, border) {
        if (image.resource) {
            const angle = this.props.rotate * Math.PI / 180;
    ...
    handleMouseDown(e) {
        ...
        // later.
        e.preventDefault();
        this.setState({drag: true, mx: null, my: null})
    }
    ...
    handleMouseMove(e) {
        ...
        if (this.state.mx && this.state.my) {
            ...
            imageState.y = lastY - yDiff;
            imageState.x = lastX - xDiff;
        }
    ...
    setNewPosition(x, y, scale) {
        let imageState = this.state.image;
        //console.log('imageState1', imageState);
        imageState.y = imageState.y + y;
        imageState.x = imageState.x + x;
    ...
    render() {
        const defaultStyle = {cursor: this.state.drag ? 'grabbing' : 'grab'}
